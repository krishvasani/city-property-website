...
export function hubContent(): HubContent {
  ...
  const faq: { q: string; a: string }[] = [
    ...
    { q: 'What do homes in Adani Shantigram cost?', a: clean(`Published all-inclusive prices as of July 2026 run from ${inrShort(lowest)} (${cheapest.name}, ${cheapest.configurations[0].label}) to ${inrShort(highest)} (${dearest.name}, 6 BHK duplex). By band: ${list(priced.slice(0, 4).map((p) => `${p.name} ${priceBand(p)}`))} at the entry end; ${list(priced.slice(-3).map((p) => `${p.name} ${priceBand(p)}`))} at the top. ${projects.filter((p) => !hasPublishedPrice(p)).length ? `${list(projects.filter((p) => !hasPublishedPrice(p)).map((p) => p.name))} are quoted on request. ` : ''}${projects.filter((p) => !hasConfigPrices(p)).length ? `${list(projects.filter((p) => !hasConfigPrices(p)).map((p) => p.name))} carry a project-level band only. ` : ''}All figures move without notice and must be confirmed with the developer.`) },
...
export function projectContent(p: Project): ProjectContent {
  ...
  const whatFor = (cs: string) => p.projectType === 'plotted' ? `${p.totalUnits ? p.totalUnits + ' ' : ''}residential plots` : p.projectType === 'villa' ? `${p.totalUnits ? p.totalUnits + ' ' : ''}villas (${cs.replace(/ villa$/i, '')})` : `${cs} apartments`;
  const priceBit = band === 'On request' ? 'pricing on request' : `${band} (as of ${p.priceAsOf})`;
  const possBit = ready ? 'ready to move' : /not published/i.test(p.possession) ? 'possession not yet published' : `possession ${p.possession.replace(' (from July 2026)', '').replace(/^Approximately/, 'approx.').replace(' per the RERA declaration', '')}`;
  // Try the fullest phrasing first; drop detail (config list → type only) rather than cutting words.
  const shortConfigs = [...new Set(cfgs.map((c) => c.label.match(/\d(?:\.\d)? ?BHK|\d ?BDR/i)?.[0] ?? c.label))].join(' · ');
  const descVariants = [
    `${p.name} at Adani Shantigram, Ahmedabad: ${whatFor(configSummary(p))}, ${priceBit}, ${possBit}, RERA ${p.rera}.`,
    `${p.name} at Adani Shantigram, Ahmedabad: ${whatFor(shortConfigs)}, ${priceBit}, ${possBit}, RERA ${p.rera}.`,
    `${p.name} at Adani Shantigram, Ahmedabad: ${whatFor(shortConfigs)}, ${priceBit}, ${possBit}.`,
    `${p.name}, Adani Shantigram: ${typeLabel(p).toLowerCase()}, ${priceBit}, ${possBit}, RERA ${p.rera}.`,
  ].map(clean);
  let description = descVariants.find((v) => v.length <= 155) ?? cutAtWord(descVariants[3], 154) + '.';
  ...
  if (priced.length) {
    ...
    o.push(clean(`Configurations on offer are ${list(cfgs.map((c) => c.label))}${size ? `, with stated areas from ${size}` : ''}. The project's price band as of ${p.priceRange.asOf} is ${band}; the developer does not publish a price per configuration, so ask us for the current quote on the specific unit you want. ${p.carpetRange ? `Carpet areas are stated only as an overall range of ${p.carpetRange}.` : ''}`));
  } else {
    o.push(clean(`Configurations on offer are ${list(cfgs.map((c) => c.label))}${size ? `, with stated areas from ${size}` : ''}. The developer has not published prices for this project; quotes are on request. ${p.carpetRange ? `Carpet areas are stated only as an overall range of ${p.carpetRange}.` : ''}`));
  }
  ...
  if (lo !== undefined) {
    graph.push({
      ...
      offerCount: priced.length || cfgs.length,
      itemOffered: { '@id': projId },
      offeredBy: { '@id': DEV_ID },
      validFrom: '2026-07-01',
      description: `All-inclusive price band as of ${p.priceAsOf} (${p.priceSource}); subject to change.`,
    });
  }
