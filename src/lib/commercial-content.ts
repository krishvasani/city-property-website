...
import {
  commercialProjects, commercialPath, isReady, possessionLabel, rate, rateFloor,
  rateLine, sizeRange, useSummary, type CommercialProject,
...
export function projectContent(p: CommercialProject): ProjectContent {
  ...
  const faq: { q: string; a: string }[] = [
    ...
    },
    {
      ...
      a: clean(`Yes. We arrange site visits, confirm current availability and rates with ${p.developer}, and handle the paperwork. We are an independent consultant registered with Gujarat RERA (${site.rera}) — we are not the developer.`),
    },
  ];
  ...
  const place: Record<string, any> = {
    ...
    name: p.name,
    url,
    description: clean(`${p.floors} ${uses.toLowerCase()} development by ${p.developer} at ${p.localityName}, Ahmedabad.`),
    ...
    additionalProperty: [
      { '@type': 'PropertyValue', name: 'Floors', value: p.floors },
  ...
  return {
    ...
    eyebrow: `${p.developer} · ${p.localityName}, Ahmedabad${p.rera ? ` · Gujarat RERA ${p.rera}` : ''}`,
